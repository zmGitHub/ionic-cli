var Cli = module.exports,
    ...
    Ionic2Project = IonicAppLib.v2.project,
...
Cli.run = function run(processArgv) {
  try {
    ...
    if (isIonicV2) {
      ...
      // task for now
      argv.v2 = true;
      var config = require(path.resolve(process.cwd() + path.sep + Ionic2Project.CONFIG_FILE));
      Cli.runHook('before', taskSetting.name, config, argv);
    }

    var promise = taskInstance.run(Cli, argv);
    if (isIonicV2){
      promise.then(function(){
        Cli.runHook('after', taskSetting.name, config, argv);
      });
    }
...
Cli.runHook = function runBeforeTaskHook(hookType, taskName, projectConfig, argv) {
  if (!hookType || !taskName || !projectConfig || !projectConfig.hooks) return;

  var hookName = hookType + taskName[0].toUpperCase() + taskName.slice(1);
  var hook = projectConfig.hooks[hookName];
  if (hook) {
    return hook(argv);
  } else {
    return null;
