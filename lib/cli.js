...
Cli.run = function run(processArgv) {
  ...
  try {
    ...
    var shouldDoRuntimeCheck = Cli.doRuntimeCheck(settings.version);

    // Special case, if they need to do a runtime check from new version
    // and they are already running the info command, let them.
    if (shouldDoRuntimeCheck && argv._[0] != 'info' ) {
      Info.checkRuntime();
    }
...
Cli.doRuntimeCheck = function doRuntimeCheck(version) {
  ...
  var versionHasBeenChecked = false;

  logging.logger.debug('Version:', version, 'lastVersionChecked:', lastVersionChecked);
  ...
  var shouldDoRuntimeCheck = !lastVersionChecked || !versionHasBeenChecked;

  if (shouldDoRuntimeCheck) {
    IonicConfig.set('lastVersionChecked', version);
    IonicConfig.save();
  }

  return shouldDoRuntimeCheck;
};
