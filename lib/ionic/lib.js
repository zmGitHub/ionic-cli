...
IonicTask.prototype.updateFiles = function() {
  ...
  try {
    ...
    writeStream.on('close', function() {
      try {
        shelljs.rm('-rf', self.tmpZipPath)
      } catch(e){
        console.error("Error removing zip:", e);
      }
      ...
      try {
        ...
        shelljs.cp('-Rf', copyFrom, ionicLibDir);

        try {
          shelljs.rm('-rf', self.tmpExtractPath)
        } catch(e){
          console.error("Error removing extract:", e);
        }

        try {
          shelljs.rm('-rf', path.join(ionicLibDir, 'README.md'));
        } catch(e){
          console.error("Error removing readme:", e);
        }

        try {
          shelljs.rm('-rf', path.join(ionicLibDir, 'bower.json'));
        } catch(e){
          console.error("Error removing bower file:", e);
        }
