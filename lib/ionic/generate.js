  ...
  */
  try {
    ...
    try {
      ...
      if (listing) {
        for (gen in ionicGenerateModule.generators) {
          console.log(gen, '\t\tionic g', gen ,'<name>');
        }
      } else {
        return ionicGenerateModule.generate({appDirectory: process.cwd(), generatorName: generator, name: name});
      }
    } catch (ex) {
      // console.log('ex', ex);
