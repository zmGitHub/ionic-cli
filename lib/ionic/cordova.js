...
IonicTask.prototype.run = function(ionic, argv) {
  ...
  var self = this,
      ...
      q, 
      platformInstalled = self.checkIfPlatformInstalled(argv),
  ...
  try {
    ...
    if (argv._.indexOf('run') != -1) {
      ...
      if (!platformInstalled) {
        var notInstalledMsg = ['• You\'re trying to run ', runPlatform, ' and you dont have it installed.'].join('').yellow,
      ...
      if (!platformInstalled || !pluginsInstalled) {
        q = self.promptForPlugins(argv);
      } else {
        q = Q(false);
      }
    } else {
...
IonicTask.prototype.runCordova = function(cmdName, argv) {
  ...
  Logging.logger.debug('Executing cordova cli: ' + cleanArgs);
...
IonicTask.prototype.checkIfPlatformInstalled = function checkIfPlatformInstalled(argv) {
  var platformPath = path.join(process.cwd(), 'platforms', argv._[1]);
  var platformStats;
  try {
...
IonicTask.prototype.promptForPlugins = function promptForPlugins(argv) {
  var q = Q.defer();

  Logging.logger.info('\nWould you like to add several plugins that will enhance your Ionic application?');
  ...
  prompt.get({properties: promptProperties}, function (err, promptResult) {
    ...
    var areYouSure = promptResult.shouldAdd.toLowerCase().trim();
