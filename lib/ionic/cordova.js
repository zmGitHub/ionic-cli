...
IonicTask.prototype.run = function(ionic, argv) {
  ...
  try {
    ...
    if (argv._.indexOf('run') != -1) {
      ...
      var platformInstalled = self.checkIfPlatformInstalled(argv);
      if (!platformInstalled) {
        var installMsg = ('\n• You\'re trying to run ' + runPlatform + ' and you dont have it installed. ').yellow + ('\n\n∆ Installing ' + runPlatform + ' for you.').blue;
        console.log(installMsg);
        shelljs.exec('ionic platform add ' + runPlatform);
        console.log(['\n√ Installed platform ', runPlatform, ', now running, enjoy!\n'].join('').green);
      }
