...
IonicTask.prototype.run = function(ionic, argv) {
  ...
  try {
    ...
    .then(function(saveResponse) {
      var argumentName = argv._[2];
      if (argumentName == 'android' && isAddCmd && !argv.nocrosswalk) {
        console.log('Adding in Crosswalk by default (specify --nocrosswalk to ignore)'.yellow.bold);
        return self.runCordova('plugin', { _: ['plugin', 'add', 'cordova-plugin-crosswalk-webview']});
      }
    })
    .catch(function(ex) {
...
IonicTask.prototype.runCordova = function(cmdName, argv) {
  ...
  // var cmdArgsOld = (process.argv.length > 3 ? process.argv.slice(3) : []);
  var cmdArgs = (argv._.length > 2 ? argv._.slice(1) : []);
  var cmdArg, x, y;

  // console.log('cmdName', cmdName);
  // console.log('cmdArgs', cmdArgs);
  // console.log('argv', argv);
  // console.log('cmdArgsOld', cmdArgsOld);
