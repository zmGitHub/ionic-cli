...
IonicTask.prototype._postToLiveReload = function(files) {
  var postRequestUrl = ['http://', this.address, ':', this.liveReloadPort, '/changed'].join('');
  console.log('postRequestUrl', postRequestUrl, 'for files:' , files)
  request.post(postRequestUrl, {
    path: '/changed',
    method: 'POST',
    body: JSON.stringify({
      files: files
  ...
  // console.log('Sent post request changed')

};

//Note - this is the live reload for `file://` - only works with iOS
IonicTask.prototype.setupLiveReload = function() {
  ...
  var self = this;
  // self.address = '192.168.1.106';
  self.address = '192.168.1.106';
  self.liveReloadPort = '35729';
  self.liveReloadServer = ['http://', self.address, ':', self.liveReloadPort].join('')
  self.watchPatterns = project.get('watchPatterns') || ['www/**/*', '!www/lib/**/*'];


  vfs.watch(self.watchPatterns, {},function(f) {
    // self._changed(f.path);

    // '/Users/JoshBavari/Development/testing/j3/www/templates/tab-dash.html' }
    // process.cwd /Users/JoshBavari/Development/testing/j3
    var currPath = path.resolve('www');

    // f.forEach(function(file) {
    f.path = path.relative(currPath, f.path);
    console.log('new f path:', f.path)
    // })
    // console.log('Files changed:', f)
    console.log('process.cwd', process.cwd())
    self.copyFiles();
    self._postToLiveReload(f.path);
  });
...
IonicTask.prototype.copyFiles = function() {
  //Grab www
  //Look up device ID
  //Get App ID - find . 'projectname.app'
  //Copy files to ~/Library
  console.log('Copying files')
  var iosSimulatorPath = '/Users/JoshBavari/Library/Developer/CoreSimulator/Devices/';
  var iosDeviceId = '4A823282-0747-4E42-A060-7F54BB602806'; //project.get('iosDeviceId');
  //Get this by going to iOS Simulator > Menu > Hardware > Device > Manage Devices
  //You can find the device ID here by finding it from the list of devices on the side
  var iosSimulatorDeviceApplicationPath = '/data/Containers/Bundle/Application';

  var iosAppPath = [iosSimulatorPath, iosDeviceId, iosSimulatorDeviceApplicationPath].join('');

  //Get App id
  var appName = ['j3', '.app'].join('');
  var findCommand = ['find', iosAppPath, '-name', appName].join(' ');
  console.log('find command:', findCommand);
  var cmdResult = shelljs.exec(findCommand);
  //output has our result: 
  ///Users/JoshBavari/Library/Developer/CoreSimulator/Devices/4A823282-0747-4E42-A060-7F54BB602806/data/Containers/Bundle/Application//E80E82BD-AB28-4B14-8A01-D0A23BBD15FC/j3.app
  var pathChunks = cmdResult.output.split('/')
  var appId = pathChunks[pathChunks.length - 2];

  console.log('App ID', appId)

  var lookedUpAppPath = [iosAppPath, '/', appId, '/', appName, '/', 'www'].join('');
  // shelljs.rm('-R', path.join(lookedUpAppPath, '*'));

  //941C0C3E-81C2-43D3-A878-D9268E1C0AD5
  try {
    shelljs.cp('-Rf', path.resolve('www', '*'), lookedUpAppPath);  
  }catch(ex){
