...
exports.IonicStats = {
  t: function(additionalData) {
    try {
      ...
      try {
        ...
        console.error('Error retrieving ionic version:', e2);
      }
      ...
      try {
        ...
        console.error('Error retrieving cli version:', e2);
      }

      try {
        statsData.email = ionicConfig.get('email');
      } catch (e2) {
        console.error('Error retrieving email fromc config:', e2);
      }

      try {
        statsData.account_id = ionicConfig.get('id');
      } catch (e2) {
        console.error('Error retrieving id fromc config:', e2);
      }
  ...
  mp: function(e, d) {
    ...
    mixpanel.track(e, d, function(err) {
      if (err) {
        console.error(err);
      }
    });
