...
IonicTask.prototype.run = function(ionic) {
  ...
  login.get(ionic, function(jar) {
    ...
    } else if (argv._[1] && argv._[1].trim() == 'google-api-key') {
      if (argv._[2]) {
        self.set_google_api_key(self, argv._[2].trim());
      } else {
        console.log('You need to specify a Google API Key!'.bold.red);
      }
...
IonicTask.prototype.send_push = function(self) {
  ...
  promptProperties['push-api-key'] = {
    name: 'push-api-key',
    description: 'What is your push API key'.yellow.bold,
    required: true,
    isFile: false
  };
  ...
  promptProperties['device-token'] = {
    name: 'device-token',
    description: 'What device token do you want to send to'.yellow.bold,
    required: true,
    isFile: false
  }
  promptProperties['platform'] = {
    name: 'platform',
    description: 'What platform is the device? (android or ios)'.yellow.bold,
    required: true,
    isFile: false
  };
  ...
  prompt.get({properties: promptProperties}, function(err, promptResult) {
    ...
    var notification = {
      ...
      notification: {
        ...
        ios: {
          badge: parseInt(promptResult['badge']) || null,
    ...
    request(options, function(err, response, body) {
      if (!err && response.statusCode == 202) {
        console.log("Successfully queued push notification. The message tracking id is:", body.message_id);
      } else {
