...
IonicTask.prototype.promptLogin = function() {
  console.log('\nWould you like to create an ionic.io account to use the View app and other services?');
  ...
  prompt.get({properties: promptProperties}, function (err, promptResult) {
    console.log('YOU TYPED', promptResult);
    if(err) {
      return console.log(err);
    }
...
IonicTask.prototype.finalize = function() {
  ...
  this.ionic.printNewsUpdates(true).then(function() {
    self.promptLogin();
  });
