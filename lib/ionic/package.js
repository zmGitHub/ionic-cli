...
IonicTask.prototype.buildPostRequest = function(platform) {
  ...
  csrfmiddlewaretoken = IonicUtils.retrieveCsrfToken(this.jar);
...
IonicTask.prototype.submitPostRequest = function(form, platform) {
  ...
  var headersToTransform = {cookie: IonicUtils.transformCookies(self.jar)};

  console.log((platform + ' ' + self.mode + ' building...').bold);
  if(privateData.get('cck')) {
    headersToTransform.cck = privateData.get('cck');
  }
  ...
  form.submit({
    ...
    port: params.port,
    path: params.path,
    headers: form.getHeaders(headersToTransform)
  },
